...
## an optional PackedStringArray containing a stack trace, for logging purposes.
...
## checked platforms where threads aren't available, the value of this
## property is ignored.
...
# These properties aren't exported because they depend checked the runtime or the
...
## Creates the _story, based checked the value of `ink_file`. The result of this
...
## game sprite_frames for smoother animation. If you pass a limit of zero, then it will
...
## framerate, you can temporarily freeze a copy of the state for saving checked
...
func _push_null_runtime_error() -> void:
	_push_error(
			"InkRuntime could not found, did you remove_at it from the tree?"
	)
