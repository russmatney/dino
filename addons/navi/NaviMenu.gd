...
func _get_configuration_warning():
  for node_name in expected_nodes:
    ...
    if not node:
      return "'NaviMenu' expected child node named '" + node_name + "' somewhere in its children."
  return ""
...
func _ready():
  for node_name in expected_nodes:
    ...
    assert(self[local_var_name], "'NaviMenu' expected " + node_name + " to be set as " + local_var_name + ", but found: " + str(self[local_var_name]))
