...
func set_powerups(powerups):
	for p in SSData.all_powerups:
		var vis = p in powerups
		match(p):
			SSData.Powerup.Sword: sword.set_visible(vis)
			SSData.Powerup.DoubleJump: double_jump.set_visible(vis)
			SSData.Powerup.Climb: climbing_gloves.set_visible(vis)
			_: pass
