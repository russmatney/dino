...
The Godot 4 port is complete, with one or two exceptions - TowerJet's tilemaps
aren't quite right, but those are going to be dropped/refactored anyway.

--

NOTE: games that use the ActionHint (unfortunately most of them) will not work
with naive clone just yet, because I'm not yet including some game assets
(third-party fonts) in the repo. I'm hopeful to implement a technical solution
to this, like working using a fallback font when the referenced one is missing.
...
A work-in-progress for the [[https://itch.io/jam/godot-wild-jam-58][Godot Wild Jam #58]]
** Mountain:
A work-in-progress.
